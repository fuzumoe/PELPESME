...
public class SPARQLderivedPointExtractor {

	// takes an ontology-file and returns all derived points in a matrix
	public Object[][] extract(String file) {
		...
		String query = "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> "
				...
				+ "SELECT ?x ?subjectX ?ValueX ?subjectY ?ValueY ?subjectZ ?ValueZ " + "WHERE {" +

				"?x rdf:type :Ordered_List_Derived_Cartesian_Point_Coordinates."
				+ "?x :ordered_list_has_Slot ?subjectX." + "?subjectX rdf:type :Cartesian_X_Coordinate."
				+ "?subjectX :hasValue ?ValueX." +

				"?x :ordered_list_has_Slot ?subjectY." + "?subjectY rdf:type :Cartesian_Y_Coordinate."
				+ "?subjectY :hasValue ?ValueY." +

				"?x :ordered_list_has_Slot ?subjectZ." + "?subjectZ rdf:type :Cartesian_Z_Coordinate."
		...
		// or return even a rewindable result set, so that we can iterate
		// through it to get the size and reset is afterwards
		...
		for (; rsrw.hasNext();) {
			...
			Literal x = soln.getLiteral("ValueX"); // Get a result variable by
													// name.
			...
			System.out.println("point[" + i + "]" + "[" + point[i][0] + "]" + "[" + point[i][1] + "]" + "["
					+ point[i][2] + "]" + "[" + point[i][3] + "]");
	...
	public static void main(String[] args) {
		SPARQLderivedPointExtractor test = new SPARQLderivedPointExtractor();
		test.extract("file:c:/users/max/desktop/PELOPS/10303_108_Populated_Derived2.owl");
	}
